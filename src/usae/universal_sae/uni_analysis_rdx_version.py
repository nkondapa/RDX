...
class ModelActivationExtractor:
    ...
    def __init__(self, activations, model_zoo):
        """
        Initialize the activation extractor.
    ...
    def extract_all_activations(self) -> Dict[str, torch.Tensor]:
        ...
        return self.model_activations, None
...
class SAEAnalyzer:
    ...
    def _compute_model_stats(self, model_name: str, activations: torch.Tensor) -> Dict:
        ...
        total_samples = activations.shape[0] * num_tokens
...
class ActivationAnalyzer:
    ...
    def compute_firing_entropy(self, model_sums: Dict[str, torch.Tensor],
        ...
        return dict(entropy=entropy, prob_dist=prob_dist)
...
def run_full_analysis_pipeline(params, activations, model_zoo: Dict) -> Dict:
    """
    ...
    activation_analyzer = ActivationAnalyzer(sae_results, params, {'batch_size': 32}, model_zoo)
    ...
    firing_entropy, prob_dist = final_results['firing_entropy']['entropy'], final_results['firing_entropy']['prob_dist']
    # plt.figure()
    # plt.hist(firing_entropy.numpy(), bins=30, color='skyblue', edgecolor='black')
    # plt.title('Histogram of Firing Entropy Across Concepts', fontsize=16)
    # plt.xlabel('Firing Entropy', fontsize=14)
    ...
    for i, idx in enumerate(best_ids):
        ...
        if heatmaps != None:
            ...
            if image_save_path != None:
                ...
                save_subplot_overlay(image, heatmap, cmap=cmap, alpha=alpha, save_path=save_name,
                                     model_name=model_types[model_num])
