...
def multiplicative_update(
    A, Z, D, update_Z=True, update_D=True, max_iter=500, tol=1e-5
):
    """
...
def projected_gradient_descent(
    A, Z, D, lr=5e-2, update_Z=True, update_D=True, max_iter=500, tol=1e-5
):
    """
...
class NMF(BaseOptimDictionaryLearning):
    ...
    def encode(self, A, max_iter=300, tol=None):
        ...
        Z, _ = self.solver_fn(
            A, Z, self.D, update_Z=True, update_D=False, max_iter=max_iter, tol=tol
        )
    ...
    def init_nndsvda(self, A):
        ...
        """
        Z, D = _initialize_nmf(
            A.detach().cpu().numpy(), self.n_components, init="nndsvda"
        )
