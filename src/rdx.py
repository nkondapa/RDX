...
class RDX:
    ...
    @staticmethod
    def apply_diff_function(params, r1_dm, r0_dm, diff_function, gamma, normalize_diff_mat_by_abs_max):

        symmetrize_dm = params.get('symmetrize_dm', None)
        if symmetrize_dm == 'mean':
            r0_dm = (r0_dm + r0_dm.T) / 2
            r1_dm = (r1_dm + r1_dm.T) / 2
        elif symmetrize_dm == 'harmonic_mean':
            r0_dm = 1 / (1 / r0_dm + 1 / r0_dm.T)
            r1_dm = 1 / (1 / r1_dm + 1 / r1_dm.T)
        elif symmetrize_dm == 'max':
            r0_dm = torch.max(r0_dm, r0_dm.T)
            r1_dm = torch.max(r1_dm, r1_dm.T)
        elif symmetrize_dm == 'min':
            r0_dm = torch.min(r0_dm, r0_dm.T)
            r1_dm = torch.min(r1_dm, r1_dm.T)
    ...
    def construct_graph(repr0, repr1, params):
        ...
        if sim_function == 'neighborhood':
            ...
            diff_10, diff_01 = RDX.apply_diff_function(params, r1_dm, r0_dm, diff_function, gamma, normalize_diff_mat_by_abs_max)
            ...
            return dict(am_10=am_10, am_01=am_01, dm_10=dm_10, dm_01=dm_01, r0_am=r0_am, r1_am=r1_am, r0_dm=r0_dm,
                        r1_dm=r1_dm, diff_10=diff_10, diff_01=diff_01, r0_sort=r0_sort, r1_sort=r1_sort)
            ...
            return dict(am_10=am_10, am_01=am_01, dm_10=dm_10, dm_01=dm_01, r0_am=r0_am, r1_am=r1_am, r0_dm=r0_dm,
                        r1_dm=r1_dm, diff_10=diff_10, diff_01=diff_01)
    ...
    def cluster_graph(graph_dict, cluster_params):
        ...
        for graph_key in graph_keys:
            ...
            if method == 'spectral':
                gk = 'am_' + graph_key
                am = graph_dict[gk]
                RDX.pre_process(am, cluster_params.get('preprocess_clustering', {}))
                cluster = SpectralClustering(n_clusters=n_clusters + int(add_null_cluster), affinity='precomputed')

                if not torch.allclose(am, am.T):
                    am = (am + am.T) / 2

                cluster.fit(am)
                RDX.post_process(cluster, graph_dict[gk], thresh=cluster_params.get('filter_thresh', -1))
    ...
    def post_process(cl, am, thresh=-1):
        ...
        for i, cli in enumerate(sorted_clusters):
            if cluster_aff_mean[cli] < thresh:
                ...
                new_labels[labels == cli] = i + 1
        cl.labels_ = new_labels


    @staticmethod
    def pre_process(am, params):
        if params.get('clip_below_thresh', False):
            thresh = params['clip_thresh']
            am[am < thresh] = params['clip_fill_val']
