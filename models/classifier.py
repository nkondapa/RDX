...
class ClassificationModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, factor=0.5, patience=1)
        # optimizer = torch.optim.SGD(self.parameters(), lr=lr, momentum=0.9)
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=lr, epochs=num_epochs,
                                                           steps_per_epoch=steps_per_epoch)
        lr_scheduler_config = {'scheduler': lr_scheduler, 'interval': 'step', 'frequency': 1} #'monitor': 'train_loss'
