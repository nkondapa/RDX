...
class MNISTDataset(VisionDataset):
    ...
    def __init__(
        ...
        if self._check_legacy_exist():
            self.data, self.labels = self._load_legacy_data()
            self.data = self.data.type(torch.float).unsqueeze(1)
            self.paths = np.array([f'root/{lab}/{i}' for i, lab in enumerate(self.labels)])
            self.process_mod_params()
            return
    ...
    def process_mod_params(self):

        vflip = self.mod_params.get('hflip', False)
        hflip = self.mod_params.get('vflip', False)
        classes = self.mod_params.get('classes', None)
        new_labels = self.mod_params.get('new_labels', False)
        if hflip:
            # flip every image in data horizontally and concatenate
            flip_data = T.functional.vflip(self.data) # for some reasons vflip does horizontal flip in this dataset
            # import matplotlib.pyplot as plt
            # fig, axes = plt.subplots(1, 2)
            # axes[0].imshow(self.data[0][0], cmap='gray')
            # axes[0].set_title('Original')
            # axes[1].imshow(flip_data[0][0], cmap='gray')
            # axes[1].set_title('Flipped')
            # plt.show()

            self.data = torch.cat((self.data, flip_data), dim=0)
        if vflip:
            # flip every image in data vertically and concatenate
            flip_data = T.functional.hflip(self.data)
            self.data = torch.cat((self.data, flip_data), dim=0)

        if classes:
            # filter data and labels based on classes
            mask = torch.zeros(self.data.shape[0], dtype=torch.bool)
            for c in classes:
                mask |= (self.labels == c)
        ...
        if self.data.shape[0] != self.labels.shape[0]:
            if new_labels:
                nl_labels = torch.arange(10, 20)
                map_dict = {i: j for i, j in zip(torch.arange(10).tolist(), nl_labels.tolist())}
                new_labels = torch.tensor([map_dict[i.item()] for i in self.labels])
