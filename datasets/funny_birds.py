...
class FunnyBirds(Dataset):
    ...
    def __init__(self, root_dir, mode, get_part_map=False, transform=None):
        ...
        with open(os.path.join(self.root_dir, f'parts.json')) as f:
        ...
        self.bg_color = (0,0,0)
        self.labels = [entry['class_idx'] for entry in self.params]
        self.paths = [os.path.join(self.mode, str(entry['class_idx']), str(idx).zfill(6)+'.png') for idx, entry in enumerate(self.params)]
    ...
    def __getitem__(self, idx):
        ...
        #           'input': image, 'target': class_idx, 'path': img_path}
        return dict(input=image, target=class_idx, path=img_path)
