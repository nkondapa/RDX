...
def get_transform(params):
    ...
    if dataset_name == 'chinese_chars' or dataset_name == 'butterflies' or dataset_name.split('_')[0] == 'butterflies':
        offline_triplet = params.get('offline_triplet', False)
        use_test_transforms_for_train = params.get('use_test_transforms_for_train', False)
        if offline_triplet or use_test_transforms_for_train:
            print('Using test transforms for training')
            transform = T.Compose([
                T.Resize((224, 224)),
                T.ToTensor(),
                T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    if dataset_name == 'ham10000':
        backbone_name = params.get('backbone_name', 'ham10000_inception')
