...
class CUBDataset(Dataset):
    ...
    def __init__(self, pkl_file_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, num_classes, transform=None, pkl_itself=None):
        ...
        idx = self.data[0]['img_path'].split('/').index('CUB_200_2011')
        self.paths = []
        self.labels = []
        for i in range(len(self.data)):
            self.paths.append('/'.join(self.data[i]['img_path'].split('/')[idx + 1:]))
            self.labels.append(self.data[i]['class_label'])
        self.labels = np.array(self.labels)
        self.paths = np.array(self.paths)
        self.data_root = self.image_dir
